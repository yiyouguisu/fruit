...
class DbController extends CommonController {
    ...
    public function index(){
        $Db    = Db::getInstance();
        $all_table_info  = $Db->query("SHOW TABLE STATUS like 'zz_%'");
        $this->assign("tablelist",$all_table_info);//数据库内所有表信息
        $this->display();
    }
    //查看表结构
    public function showtable(){
      $table=str_replace(";","",I('table'));
      //数据表结构
      //设置引号
    ...
    public function downzip(){
      $dbpath = realpath(C('DATA_BACKUP_PATH')) . DIRECTORY_SEPARATOR;
      $file_name=I('time');
      $compress=I('compress');
      $part=I('part');
      if($compress=='GZ'){
        $file_name =date('Ymd-His', $file_name) . "-" . $part . ".sql." . strtolower($compress);
      }else if($compress=='-'){
        $file_name =date('Ymd-His', $file_name) . "-" . $part . ".sql";
      }
      if(!file_exists($dbpath.$file_name))   {   //检查文件是否存在  
          $this->error("文件找不到");
      }else{  
        $file = fopen($dbpath.$file_name,"r"); // 打开文件
        // 输入文件标签
        header("Content-type: application/octet-stream");
        header("Accept-Ranges: bytes");
        header("Accept-Length: ".filesize($dbpath.$file_name));
        header("Content-Disposition: attachment; filename=" . $file_name);
        // 输出文件内容
        echo fread($file,filesize($dbpath.$file_name));
        fclose($file);
        //@unlink($dbpath."/".$file_name);
        exit();
      }
    }
