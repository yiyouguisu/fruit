...
class IntegralshareController extends CommonController {
    ...
    public function index() {
        ...
        $count = M("Integralshare")->where($where)->count();
        $page = new \Think\Page($count,10);
        $data = M("Integralshare")->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
    ...
    public function edit() {
        if (IS_POST) {
            $step=I('addintegral');
            $integral= M("Integralshare")->where('id=' . $_POST['id'])->find();
            $id = M("Integralshare")->where('id=' . $_POST['id'])->save(array(
            ...
            $data = D("Integralshare")->where(array("id" => $_GET["id"]))->find();
            $this->assign("data", $data);
