...
<head>
    ...
    <script type="text/javascript">
        ...
        template_all += '              <span>{lastday}｜<span>{pnum}人已购买</span>                                                                                    ';
        template_all += '           </div>                                                                                                                                       ';
        template_all += '        </div>                                                                                                                                          ';
        ...
        template_all += '       <span style="float:left; "><img alt="" src="__IMG__/orderStyleTg.png" /></span>                                                                  ';
        ...
        function replaceData(response) {
            $.each(response, function (key, value) {
                ...
                if (value.isover == '1') {
                    template = template.replace('{lastday}', '已过期');
                    template = template.replace('{isread}', "display:none");
                    template = template.replace('{noreads}', "display:none");
                } else {
                    template = template.replace('{lastday}', '还剩' + value.lday + '天' + value.lhour + '小时' + value.lminit + '分');
                }
                if (value.stock == '0') {
                    template = template.replace('{isread}', "display:none");
                }
