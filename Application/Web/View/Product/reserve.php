...
<head>
    ...
    <script type="text/javascript">
        ...
        var template_all = ' <div class="groupGoodsItem">                                                                                                                           ';
        template_all += '    <div class="content">                                                                                                                               ';
        template_all += '        <div class="infos">                                                                                                                             ';
        template_all += '           <a href="{productview_img}" target="_self"><div class="title">{title}</div>                                                      ';
        ...
        template_all += '           </div>                                                                                                                                       ';
        template_all += '           <div class="mdate">                                                                                                                          ';
        template_all += '              <span>{slodtime}｜<span>{pnum}人已购买</span>                                                                                    ';
        template_all += '           </div>                                                                                                                                       ';
        template_all += '        </div>                                                                                                                                          ';
        ...
        template_all += '        </div>                                                                                                                                          ';
        template_all += '    </div>     </a>                                                                                                                                         ';
        template_all += '    <div class="numbers" >                                                                                                                               ';
        template_all += '       <span style="float:left; "><img alt="" src="__IMG__/orderStyleYg.png" /></span>                                                                  ';
        template_all += '       <span style="float:right;{isread}" ><span class="goodsCountCut" {isCutdis}></span><label class="goodsCountLab" id="{goods_id}" {isLabdis}>{cartnum}</label><span class="goodsCountAdd"></span></span>  ';
        template_all += '       <span style="float:right;color:red;{noreads}" >正在补货中...</span>  ';
        template_all += '    </div>                                                                                                                                              ';
        template_all += ' </div>         ';
        ...
        function replaceData(response) {
            $.each(response, function (key, value) {
                ...
                if (value.isover == '1') {
                    template = template.replace('{slodtime}', '已过期');
                    template = template.replace('{isread}', "display:none");
                    template = template.replace('{noreads}', "display:none");

                } else {
                    template = template.replace('{slodtime}', '出售时间：' + value.slodtime);
                }
                if (value.stock == '0') {
                    template = template.replace('{isread}', "display:none");
                }
...
<body>
    <div id="page_head" class="page_head">
        <div class="l"><a id="toolReturn" class="return" href="{:U('Web/Index/index')}"></a></div>
        <h1>预购</h1>
    </div>
