                    ...
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="productshow" data-id="<?php echo ($vo["id"]); ?>">
                            ...
                            <td align="center" > 
                                <?php if($vo['status'] != 5): if(empty($vo['puid'])): ?><a href="javascript:;" onClick="omnipotent('selectid','<?php echo U('Admin/Order/deal',array('orderid'=>$vo['orderid']));?>','派发订单',1,300,180)">派发</a></br>
                                        ...
                                        <?php if(($vo['package_status']) == "2"): ?>【<?php echo getAuser($vo['puid']);?>】包装完成</br><?php endif; endif; ?>
                                    <?php if(empty($vo['ruid'])): ?><a href="<?php echo U('Admin/Order/dodistribute',array('orderid'=>$vo['orderid']));?>">提示送货员</a></br>
                                        <?php else: ?>
                                        ...
                                        <?php if(($vo['delivery_status']) == "4"): ?>【<?php echo getuser($vo['ruid']);?>】配送完成</br><?php endif; endif; ?>
                                    <?php else: ?>
