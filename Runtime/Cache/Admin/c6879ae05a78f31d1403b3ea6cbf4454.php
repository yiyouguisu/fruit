...
<script>
    ...
    <script type="text/javascript">
        ...
        if(role==2){
            var url="<?php echo U('Admin/Package/ajax_getneworder');?>";
            window.setInterval(function(){
                getneworder(url)
            },5000);
        }else if(role==6){
            var url="<?php echo U('Admin/Package/ajax_getnewerrororder');?>";
            window.setInterval(function(){
                getneworder(url)
            },5000);
        }else if(role==3){
            var url="<?php echo U('Admin/Order/ajax_getneworder');?>";
            window.setInterval(function(){
                getneworder(url)
            },5000);
        }else if(role==1){
            var url="<?php echo U('Admin/Order/ajax_getnewcompanyorder');?>";  
            window.setInterval(function(){
                getneworder(url)
            },5000);        
        }
                            ...
                            if (ps) {
                                if(ps.split(',').indexOf(data.order.orderid+'')==-1){
                                    ...
                                    layer.open({
                                        ...
                                        shade: false,
                                        area: ['330px','220px'],
                                        content: "<?php echo U('Admin/Order/newordernotice');?>?orderid=" + data.order.orderid,
                                        end:function(){
                                            isopen=0;
                                            if(role==2){
                                                iframeJudge({
                                                    elem: $(this),
                                                    content:"订单包装管理",
                                                    href: "<?php echo U('Admin/Package/index',array('menuid'=>348));?>",
                                                    id: '348Admin'
                                                });
                                            }else if(role==6){
                                                iframeJudge({
                                                    elem: $(this),
                                                    content:"订单包装管理",
                                                    href: "<?php echo U('Admin/Package/errororder',array('menuid'=>422));?>",
                                                    id: '422Admin'
                                                });
                                            }
                                        }
