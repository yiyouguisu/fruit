...
<?php } ?>
 <!-- -->
  ...
  <form method="post" action="<?php echo U('Admin/Logs/index');?>">
  <div class="search_type cc mb10">
    ...
    <select class="select_2" name="status" style="width:70px;">
        <option value='' <?php if($_POST['status'] == ''): ?>selected<?php endif; ?>>不限</option>
                <option value="1" <?php if($_POST['status'] == '1'): ?>selected<?php endif; ?>>成功</option>
                <option value="2" <?php if($_POST['status'] == '2'): ?>selected<?php endif; ?>>失败</option>
              
      </select>
      用户ID：<input type="text" class="input length_2" name="uid" size='10' value="<?php echo ($_POST['uid']); ?>" placeholder="用户ID">
    ...
    时间：
      ...
      <a class="btn" name="del_log_4" href="<?php echo U('Admin/Logs/del');?>" >删除一月前数据</a>
      </span> </div>
      </form> 
  </div>
        ...
        <tbody>
          <?php if(is_array($logs)): $i = 0; $__LIST__ = $logs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td align="center">
                ...
                <?php if($vo['status'] == 2): ?><font color="#FF0000">失败</font><?php endif; ?>
          </td>
              <td align=""><?php echo ($vo["info"]); ?></td>
