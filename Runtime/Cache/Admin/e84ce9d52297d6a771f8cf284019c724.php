...
<?php } ?>
        ...
        <form method="get" action="<?php echo U('Admin/Order/waitdistribute');?>">
        ...
        <form action="<?php echo U('Admin/Order/deals');?>" method="get" >
                ...
                <table width="100%" cellspacing="0">
                 ...
                 <?php $money_total1=0;?>
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="productshow" data-id="<?php echo ($vo["id"]); ?>">
                            <td><input type="checkbox" class="J_check" data-yid="J_check_y" data-xid="J_check_x" name="ids[]" value="<?php echo ($vo["orderid"]); ?>"></td>
                            ...
                            <td align="center" > 
                                <?php if($vo['status'] != 5): if(empty($vo['puid'])): ?><a href="javascript:;" onClick="omnipotent('selectid','<?php echo U('Admin/Order/deal',array('orderid'=>$vo['orderid']));?>','派发订单',1,300,180)">派发</a></br>
                                        <?php else: ?>
                                        已派发</br><?php endif; ?>
                                    <?php else: ?>
                                    订单已完成<br/><?php echo (date("Y-m-d H:i:s",$vo["donetime"])); ?></br><?php endif; ?>
                                <a href="<?php echo U('Admin/Order/cancel',array('orderid'=>$vo['orderid']));?>" class="cancel">取消订单</a></br>
                                <a href="javascript:;" onClick="omnipotent('selectid','<?php echo U('Admin/Order/orderprint',array('orderid'=>$vo['orderid']));?>','打印订单',1,390,250)">打印</a>
                            </td>
                        </tr>
