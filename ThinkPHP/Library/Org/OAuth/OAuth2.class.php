 ...
 */
define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 7200);

/**
 * The default duration in seconds of the authorization code lifetime.
 */
define("OAUTH2_DEFAULT_AUTH_CODE_LIFETIME", 3600);
 ...
 * "access_token" but other implementations use things like "access_token".
 *
...
abstract class OAuth2 {
  ...
  /**
   * Look up the supplied access_token from storage.
   *
   * We need to retrieve access token data as we create and verify tokens.
   *
   * @param $access_token
   *   access_token to be check with.
   *
   * @return
   *   An associative array as below, and return NULL if the supplied access_token
   *   is invalid:
   ...
   */
  abstract protected function getAccessToken($access_token);
   ...
   */
  abstract protected function setAccessToken($access_token, $uid, $client_id, $refresh_token, $expires, $scope = NULL);
  ...
  public function grantAccessToken() {
    ...
    $token = $this->createAccessToken($client[0], $stored["uid"], $input["scope"]);
   ...
   * @ingroup oauth2_section_3
   */
  public function finishClientAuthorization($is_authorized, $uid, $params = array()) {
    $params += array(
      'scope' => NULL,
      'state' => NULL,
    );
    ...
    if ($is_authorized === FALSE) {
      ...
      if ($response_type == OAUTH2_AUTH_RESPONSE_TYPE_AUTH_CODE || $response_type == OAUTH2_AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
        $result["query"]["code"] = $this->createAuthCode($client_id, $redirect_uri, $uid, $scope);

      if ($response_type == OAUTH2_AUTH_RESPONSE_TYPE_ACCESS_TOKEN || $response_type == OAUTH2_AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
        $result["fragment"] = $this->createAccessToken($client_id, $uid, $scope);
    }
    return $result["query"]["code"];
   ...
   * @ingroup oauth2_section_4
   */
  protected function createAccessToken($client_id, $uid, $scope = NULL) {
    $token = array(
      "access_token" => $this->genAccessToken(),
      "expires_in" => $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME),
      "scope" => $scope
    );
    $this->setAccessToken($token["access_token"], $uid, $client_id, '', time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME), $scope);

    // Issue a refresh token also, if we support them
    if (in_array(OAUTH2_GRANT_TYPE_REFRESH_TOKEN, $this->getSupportedGrantTypes())) {
      $token["refresh_token"] = $this->genAccessToken();
      $this->setRefreshToken($token["refresh_token"], $client_id, time() + $this->getVariable('refresh_token_lifetime', OAUTH2_DEFAULT_REFRESH_TOKEN_LIFETIME), $uid, $scope);
   ...
   * @ingroup oauth2_section_3
   */
  private function createAuthCode($client_id, $redirect_uri, $uid, $scope = NULL) {
    $code = $this->genAuthCode();
    $this->setAuthCode($code, $client_id, $redirect_uri, time() + $this->getVariable('auth_code_lifetime', OAUTH2_DEFAULT_AUTH_CODE_LIFETIME), $uid, $scope);
    return $code;
  }
