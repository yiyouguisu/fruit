...
class CCPRest {
  ...
  /**
   ...
   {
       if($this->ServerIP==""){
          return json_encode(array('statusCode'=>'172004','statusMsg'=>'IP为空'));
        }
        if($this->ServerPort<=0){
          return json_encode(array('statusCode'=>'172005','statusMsg'=>'端口错误（小于等于0）'));;
        }
        if($this->SoftVersion==""){
          return json_encode(array('statusCode'=>'172013','statusMsg'=>'版本号为空'));
        } 
        if($this->SubAccountSid==""){
          return json_encode(array('statusCode'=>'172008','statusMsg'=>'子帐号为空'));
        }
        if($this->SubAccountToken==""){
          return json_encode(array('statusCode'=>'172009','statusMsg'=>'子帐号令牌为空'));
        }
        if($this->AppId==""){
          return json_encode(array('statusCode'=>'172012','statusMsg'=>'应用ID为空'));
        }  
   }
  ...
  /**
   ...
   function accAuth()
   {
       if($this->ServerIP==""){
        ...
        if($this->AccountSid==""){
          return json_encode(array('statusCode'=>'172006','statusMsg'=>'主帐号为空'));
        }
        if($this->AccountToken==""){
          return json_encode(array('statusCode'=>'172007','statusMsg'=>'主帐号令牌为空'));
        }
        if($this->AppId==""){
          return json_encode(array('statusCode'=>'172012','statusMsg'=>'应用ID为空'));
        }   
